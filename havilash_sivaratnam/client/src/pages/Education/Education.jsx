...
export default function Education() {
  function renderTimelineItems(items) {
    ...
    return items.map((o, i) => (
      <tr className='relative h-full grid gap-x-4 gap-y-0 sm:flex sm:flex-row' style={{ 'grid-template-columns': '0.2rem auto' }}>
      ...
      <div className='relative h-screen mix-blend-difference mt-48 flex justify-center'>
        <table className='relative mt-5 text-left w-full'>
          <tbody>
            ...
            <tr className='absolute h-full grid gap-x-4 gap-y-0 sm:flex sm:flex-row' style={{ 'grid-template-columns': '0.2rem auto' }}>
