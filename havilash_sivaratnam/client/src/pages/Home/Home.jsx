...
import { insertionSort, selectionSort } from '../../components/SortAlgorithm/SortFunctions';
...
export default function Home() {
  ...
  const sortIsRunningRef = useRef(false)
  const [titleIsVisible, setTitleIsVisible] = useState(false)
  const sortResetRef = useRef(null);
  const sortFuncs = [insertionSort, selectionSort]
  const [sortFunc, setSortFunc] = useState(() => insertionSort)
  ...
  useEffect(() => {
    setTitleIsVisible(!sortIsRunningRef.current)
    setTimeout(() => {
      if (!sortIsRunningRef.current) return
  ...
  return (
    <section className='section p-0 m-0'>
      <div id='home' className='home flex items-center justify-center w-full h-screen relative m-0 left-0 top-0'>
        <SortAlgorithm className='absolute h-screen w-screen top-0 left-0 -z-50 overflow-hidden' width={window.innerWidth} setCurrentSort={setCurrentSort} sortFunc={sortFunc} isRunningRef={sortIsRunningRef} resetRef={sortResetRef}/>
        <button 
          className='absolute text-sm bottom-1 right-1 z-0 button font-consolas font-semibold
          w-8 h-8 sm:w-auto sm:h-auto overflow-hidden
          ...
          onClick={() => {setSortFunc(() => sortFuncs[1])}}>
